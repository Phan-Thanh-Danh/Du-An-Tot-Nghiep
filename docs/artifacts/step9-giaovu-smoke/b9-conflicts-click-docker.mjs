...
async function main() {
  ...
  const before = await evaluate(send, `document.body?.innerText.replace(/\s+/g,' ').includes('KhÃ´ng tÃ¬m tháº¥y xung Ä‘á»™t nÃ o')`)

  const clicked = await evaluate(send, `
    (() => {
      const buttons = [...document.querySelectorAll('button')];
      const labels = buttons.map(b => (b.textContent || '').trim()).filter(Boolean);
      const target = buttons.find(b => /Kiá»ƒm tra toÃ n há»‡ thá»‘ng/.test(b.textContent));
  ...
  const i = text.indexOf('Kiá»ƒm tra xung Ä‘á»™t PhÃ¡t hiá»‡n vÃ  xá»­ lÃ½ xung Ä‘')
  const stats = i >= 0 ? text.slice(i, i + 500) : text.slice(Math.max(0, text.indexOf('Tá»”NG XUNG Äá»˜T') - 80), Math.max(0, text.indexOf('Tá»”NG XUNG Äá»˜T') - 80) + 600)
  const foundConflict = /Tá»”NG XUNG Äá»˜T [1-9]/.test(text)
