  ...
  for (const route of routes) {
    ...
    const keywords =
      route === '/staff/conflicts'
        ? ['Xung đột', 'trùng', 'Trùng', 'giảng viên', 'phòng', 'lớp']
        : route === '/staff/profile'
        ? ['Hồ sơ', 'P12', 'FPT Polytechnic', 'Thay đổi mật khẩu', 'Save']
        : ['Công bố', 'P301', 'Thứ', 'phong', 'ca']
    const hits = keywords.filter(k => text.includes(k))
    report[report.length - 1].keywordHits = hits
    console.log(`\n===== ${route} ===== keywords=${JSON.stringify(hits)}`)
  }
