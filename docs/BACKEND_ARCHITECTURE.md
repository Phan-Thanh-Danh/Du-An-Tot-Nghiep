...
- `AdminRewardCampaignsController`: CRUD metadata đợt khen thưởng Top 100 học kỳ `/api/admin/reward-campaigns`.
...
- RD1 khen thưởng - kỷ luật có constants/schema endpoint và EF foundation (`DotKhenThuong`, `MauBangKhen`, mở rộng `KhenThuong`, `HoSoKyLuat`). RD2 bổ sung `IRewardCampaignService`/`RewardCampaignService` để quản lý CRUD metadata đợt Top 100 học kỳ, validate học kỳ/cơ sở/mẫu bằng khen/JSON tiêu chí, chống trùng đợt active và ghi audit create/update/cancel. Workflow xét danh sách, sinh PDF bằng khen và xử lý kỷ luật sẽ tách service riêng ở task sau.
