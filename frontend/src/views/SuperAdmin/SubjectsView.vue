...
const filteredSubjects = computed(() => {
  ...
  return subjects.value.filter(s => {
    ...
    const matchStatus =
      ...
      (filterStatus.value === 'active' && s.conHoatDong !== false) ||
      (filterStatus.value === 'inactive' && s.conHoatDong === false)
...
async function toggleActive(s) {
  try {
    if (s.conHoatDong) {
      await subjectApi.deactivate(s.maMonHoc)
    } else {
      await subjectApi.activate(s.maMonHoc)
    }
    await fetchSubjects()
  } catch {
    /* ignore */
  ...
  <section class="space-y-4">
    <div class="surface-card border border-card rounded-2xl p-5 shadow-sm">
      <div class="flex flex-col gap-2">
        <h1 class="text-2xl font-bold text-heading">Quản lý danh mục môn học chung</h1>
        <p class="text-sm text-label max-w-2xl">
          Phần này quản lý danh mục môn học toàn hệ thống, không phải quản lý môn học trong từng lớp học phần cụ thể.
          Dùng để tạo/sửa/nháp/activate/deactivate các môn học dùng chung cho chương trình đào tạo.
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="surface-card border border-card rounded-2xl p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold text-label uppercase tracking-wide">Tổng môn học</p>
          ...
          <div class="h-10 w-10 rounded-2xl bg-(--color-info-bg) flex items-center justify-center">
            <Book class="text-(--color-info-text)" size="20" />
          </div>
        </div>
      </div>
      ...
      <div class="surface-card border border-card rounded-2xl p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold text-label uppercase tracking-wide">Đang sử dụng</p>
          ...
          <div class="h-10 w-10 rounded-2xl bg-(--color-success-bg) flex items-center justify-center">
            <Book class="text-(--color-success-text)" size="20" />
          </div>
        </div>
      </div>
      ...
      <div class="surface-card border border-card rounded-2xl p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs font-semibold text-label uppercase tracking-wide">Ngừng sử dụng</p>
          ...
          <div class="h-10 w-10 rounded-2xl bg-(--color-danger-bg) flex items-center justify-center">
            <Book class="text-(--color-danger-text)" size="20" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="surface-card border border-card rounded-2xl shadow-sm overflow-hidden">
      <div class="p-4 border-b border-default bg-(--surface-input)">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <button
            v-if="filterStatus !== 'all' || searchQuery"
            @click="clearFilters"
      ...
      <div v-else-if="error" class="p-6">
        <div class="surface-card border border-card rounded-2xl p-6 flex flex-col items-center justify-center gap-3">
          ...
          <p class="text-xs text-label">{{ error }}</p>
          <GlassButton variant="primary" class="px-4 py-2 text-xs font-bold rounded-xl mt-2" @click="fetchSubjects">
            Thử lại
          </GlassButton>
        </div>
      </div>
            ...
            >
              ...
              <td class="px-3 py-3.5">
                ...
                >
                  <span
                    ...
                    :class="s.conHoatDong ? 'bg-(--color-success-text)' : 'bg-(--color-danger-text)'"
                  />
              ...
              <td class="px-3 py-3.5">
                <div class="flex items-center justify-center gap-1">
                  <button
                    class="h-8 w-8 rounded-lg hover:bg-(--accent-primary-soft) flex items-center justify-center text-muted hover:text-(--sidebar-accent) transition-colors"
                    title="Chỉnh sửa"
                  ...
                  <button
                    class="h-8 w-8 rounded-lg hover:bg-(--color-danger-bg) flex items-center justify-center text-muted hover:text-(--color-danger-text) transition-colors"
                    :title="s.conHoatDong ? 'Ngừng sử dụng' : 'Kích hoạt'"
                    @click.stop="s.conHoatDong ? requestDelete(s) : toggleActive(s)"
                  >
                    <X size="15" />
                  </button>
                </div>
        ...
        >
          <div class="w-full max-w-lg lg-glass-strong rounded-2xl shadow-2xl border border-(--border-card) overflow-hidden" style="max-height: 90vh">
            <div class="px-6 py-4 border-b border-(--border-default) flex items-center justify-between">
              <h3 class="text-lg font-bold text-heading">{{ formMode === 'create' ? 'Thêm môn học' : 'Chỉnh sửa môn học' }}</h3>
              <button
                @click="closeForm"
                class="text-muted hover:text-heading p-1.5 rounded-lg hover:bg-(--surface-input) transition-colors"
              >
                <X size="18" />
              </button>
            </div>
            <div class="px-6 py-5 overflow-y-auto space-y-4" style="max-height: calc(90vh - 140px)">
              ...
              <div>
                <label class="block text-xs font-semibold text-label mb-1">
                  Mã môn <span class="text-(--color-danger-text)">*</span>
                </label>
                <input
                  v-model="formData.maCodeMonHoc"
                  type="text"
                  ...
                  :class="formErrors.maCodeMonHoc ? 'border-(--color-danger-text) bg-(--color-danger-bg)' : ''"
                />
                <p v-if="formErrors.maCodeMonHoc" class="mt-1 text-xs text-(--color-danger-text) font-semibold">{{ formErrors.maCodeMonHoc }}</p>
              </div>
              <div>
                <label class="block text-xs font-semibold text-label mb-1">
                  Tên môn học <span class="text-(--color-danger-text)">*</span>
                </label>
                <input
                  v-model="formData.tenMonHoc"
                  type="text"
                  placeholder="VD: Lập trình Cơ bản"
                  class="w-full h-9 px-3 bg-(--surface-input) border border-(--border-input) rounded-lg text-sm outline-none focus:ring-2 focus:ring-(--border-focus)"
                  :class="formErrors.tenMonHoc ? 'border-(--color-danger-text) bg-(--color-danger-bg)' : ''"
                />
                <p v-if="formErrors.tenMonHoc" class="mt-1 text-xs text-(--color-danger-text) font-semibold">{{ formErrors.tenMonHoc }}</p>
              </div>
              <div>
                <label class="block text-xs font-semibold text-label mb-1">
                  Số tín chỉ <span class="text-(--color-danger-text)">*</span>
                </label>
                <input
                  v-model.number="formData.soTinChi"
                  type="number"
                  min="1"
                  max="20"
                  placeholder="VD: 3"
