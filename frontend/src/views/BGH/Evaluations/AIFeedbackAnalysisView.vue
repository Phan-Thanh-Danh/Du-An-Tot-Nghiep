      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <div class="flex-1 space-y-4">
                    <div v-for="(val, key) in sentimentSummary" :key="key" class="space-y-2">
                       ...
                       <div class="h-1.5 w-full bg-(--surface-input) rounded-full overflow-hidden">
                          <div 
                            :style="{ width: `${val}%` }" 
                            :class="['h-full rounded-full', key === 'positive' ? 'bg-(--color-success-text)' : key === 'negative' ? 'bg-(--color-danger-text)' : 'bg-(--text-placeholder)']"
                          ></div>
                       </div>
                    </div>
                 </div>
        ...
        <div class="surface-card border border-card rounded-2xl p-5 overflow-hidden relative">
           <h4 class="text-sm font-semibold text-heading uppercase tracking-wide mb-8 flex items-center gap-2">
              <Zap :size="18" class="text-(--color-warning-text)" /> Chủ đề phổ biến (AI Topics)
           </h4>
           <div class="flex flex-wrap gap-3">
              <div 
                ...
                :class="[
                  ...
                  topic.sentiment === 'positive' ? 'bg-(--color-success-bg) text-(--color-success-text) border-(--color-success-text)/20 hover:bg-(--surface-input)' : 'bg-(--color-danger-bg) text-(--color-danger-text) border-(--color-danger-text)/20 hover:bg-(--surface-input)'
                ]"
              >
