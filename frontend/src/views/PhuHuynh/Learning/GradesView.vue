...
const semesters = computed(() => {
  const s = new Set(grades.value.map(g => g.semester).filter(Boolean))
  return Array.from(s)
})
...
const filteredGrades = computed(() => {
  ...
  if (selectedSemester.value) {
    result = result.filter(g => g.semester === selectedSemester.value)
  }
  if (selectedSubjectCode.value !== 'all') {
    result = result.filter(g => g.code === selectedSubjectCode.value)
  }
  return result
})
...
const subjectsInSemester = computed(() => {
  ...
  return result.filter(g => {
    ...
    seen.add(g.code)
    return true
  }).map(g => ({ code: g.code, name: g.subject }))
})

const currentGpa = computed(() => {
  const vals = grades.value.map(g => g.total).filter(t => t != null)
...
const gpaTrend = computed(() => {
  ...
  grades.value.forEach(g => {
    if (g.semester && g.total != null) {
      if (!groups[g.semester]) groups[g.semester] = []
      groups[g.semester].push(g.total)
    }
  })
  return Object.entries(groups).map(([sem, totals]) => ({
    month: sem,
    gpa: totals.reduce((a, b) => a + b, 0) / totals.length
  }))
})

const trendPoints = computed(() => {
  const trend = gpaTrend.value
...
function getStatusInfo(total) {
  if (total == null) return { label: 'Đang học', icon: 'HelpCircle', klass: 'pending' }
  return total >= 5
    ? { label: 'Đạt (Passed)', icon: 'CheckCircle', klass: 'passed' }
    : { label: 'Học lại (Failed)', icon: 'XCircle', klass: 'failed' }
}
...
onMounted(async () => {
  try {
    ...
    error.value = e.message || 'Không thể tải dữ liệu bảng điểm'
  } finally {
    loading.value = false
  }
})
      ...
      <div class="lg-card-glass p-5 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-card">
          <h3 class="text-xs font-bold text-heading uppercase tracking-wide">
            Bảng điểm chi tiết: {{ selectedSemester || 'Tất cả' }}
          </h3>
              ...
              >
                ...
                <td class="py-3 px-3 text-right">
                  <span
                    ...
                    :class="
                      getStatusInfo(grade.total).klass === 'passed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' :
                      getStatusInfo(grade.total).klass === 'failed' ? 'bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400' :
                      'bg-(--surface-input) text-label border border-(--border-card)'
                    "
                  >
                    <component :is="getStatusInfo(grade.total).icon === 'CheckCircle' ? CheckCircle : XCircle" :size="11" />
                    {{ getStatusInfo(grade.total).label }}
                  </span>
                </td>
              </tr>
            </tbody>
        ...
        <div class="lg-card-glass p-5 lg:col-span-2 space-y-4 print:hidden">
          <div class="flex items-center justify-between">
            ...
            <span class="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded">
              GPA hiện tại: {{ currentGpa }}
            </span>
          </div>

          <div v-if="trendPoints.length === 0" class="flex items-center justify-center h-44 text-xs text-muted">
            <span>Chưa có dữ liệu xu hướng qua các kỳ.</span>
          </div>
          <div v-else class="relative w-full h-44 flex items-end justify-between px-2 pt-4 mt-6 mb-8">
            <div class="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-20 z-0">
              ...
              <div class="w-full border-t border-slate-400"></div>
              <div class="w-full border-t border-slate-400"></div>
              <div class="w-full border-t border-slate-400 opacity-0"></div>
            </div>
