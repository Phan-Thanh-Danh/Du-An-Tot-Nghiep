  ...
  <div class="space-y-6 pb-10">
    ...
    <GlassPanel variant="flat" class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 relative">
      <div class="flex items-center gap-5">
        <div class="h-10 w-10 rounded-2xl bg-(--accent-primary)/10 flex items-center justify-center text-link shadow-sm">
    ...
    <div class="flex items-center justify-between gap-4">
       ...
       <div class="relative w-64">
          ...
          <input v-model="searchQuery" type="text" placeholder="Tìm đề, môn, lớp..." class="w-full rounded-xl border border-input bg-surface-input pl-11 pr-4 py-2.5 text-sm font-medium outline-none focus:border-(--accent-primary) transition-colors" />
       </div>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="(exam, index) in filteredExams" :key="exam.id"
        ...
        <div class="flex justify-between items-start mb-4">
           <div class="h-10 w-10 rounded-2xl bg-(--accent-primary)/5 flex items-center justify-center text-muted group-hover:bg-(--accent-primary)/10 group-hover:text-link transition-colors shadow-sm">
              <FileEdit :size="24" />
           </div>
        ...
        <div class="flex-1">
           ...
           <div class="mt-6 flex flex-col gap-3">
              ...
              <div class="grid grid-cols-3 gap-2 text-center">
                <div class="rounded-xl bg-(--surface-input) border border-card px-2 py-2">
                ...
                <div class="rounded-xl bg-(--color-success-bg) border border-(--color-success-bg) px-2 py-2">
                  <p class="text-[9px] font-semibold uppercase tracking-widest text-(--color-success-text)">Đã làm</p>
                  <p class="text-sm font-semibold" style="color:var(--color-success-text)">{{ exam.completedStudents }}</p>
                </div>
                <div class="rounded-xl bg-(--color-warning-bg) border border-(--color-warning-bg) px-2 py-2">
                  <p class="text-[9px] font-semibold uppercase tracking-widest text-(--color-warning-text)">Chưa</p>
              ...
              <div class="flex items-center gap-2 rounded-xl bg-(--color-info-bg) px-3 py-2 text-[10px] font-bold" style="color:var(--color-info-text);border:1px solid var(--color-info-bg)">
                <ShieldCheck :size="13" />
  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div v-if="isConfigModalOpen" class="fixed inset-0 z-[999] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-(--overlay-bg) backdrop-blur-sm" @click="isConfigModalOpen = false"></div>
        <div class="relative surface-card rounded-2xl shadow-(--lg-shadow-md) w-full max-w-2xl overflow-hidden animate-modal-in">
          <div class="p-4 border-b border-card bg-(--surface-input) flex items-center justify-between">
            <div>
            ...
            <div class="h-10 w-10 rounded-xl bg-(--accent-primary)/10 flex items-center justify-center text-link shadow-sm">
               <Settings :size="24" />
            </div>
           </div>
           ...
           <div class="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
             <div class="space-y-4">
               ...
               <div>
                 ...
                 <input v-model="configuringExam.name" type="text" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
               </div>
               <div class="grid grid-cols-2 gap-4">
                 <div>
                   <label class="block text-sm font-semibold text-body mb-1.5">Môn học</label>
                   <input v-model="configuringExam.subjectName" type="text" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
                 </div>
                 <div>
<label class="block text-sm font-semibold text-body mb-1.5">Lớp học phần áp dụng</label>
                  ...
                  <div>
                   ...
                   <input v-model="configuringExam.date" type="text" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
                 </div>
               </div>
             </div>
...
<label class="block text-sm font-semibold text-body mb-1.5">Thời gian mở</label>
                  ...
                  <div>
                    ...
                    <select v-model="configuringExam.maxAttempts" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors">
                  ...
                  <div>
                    ...
                    <select v-model="configuringExam.status" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors">
...
<label class="block text-sm font-semibold text-body mb-1.5">SV được phép</label>
                     <input v-model.number="configuringExam.allowedStudents" type="number" min="0" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-body mb-1.5">Đã làm</label>
                     <input v-model.number="configuringExam.completedStudents" type="number" min="0" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
                  </div>
                  <div>
                    <label class="block text-sm font-semibold text-body mb-1.5">Chưa làm</label>
                    <input v-model.number="configuringExam.pendingStudents" type="number" min="0" class="w-full rounded-xl border border-input bg-surface-input px-4 py-2.5 text-sm outline-none focus:border-(--accent-primary) transition-colors" />
               </div>
                </div>

                <div class="flex items-center justify-between p-4 rounded-xl border border-card bg-(--surface-input) mt-2">
                  <div>
                  ...
                  <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div class="w-11 h-6 bg-(--border-input) peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-(--border-input) after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-link"></div>
                  </label>
               </div>
               <div class="rounded-xl border border-(--color-info-bg) bg-(--color-info-bg) p-3 text-xs font-bold leading-relaxed" style="color:var(--color-info-text)">
           ...
           <div class="p-4 border-t border-card bg-(--surface-input) flex justify-end gap-3">
