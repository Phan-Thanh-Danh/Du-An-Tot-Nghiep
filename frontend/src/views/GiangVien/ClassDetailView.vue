    ...
    <div class="surface-card border border-card rounded-2xl p-4">
      <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-5">
          <button @click="router.push('/teacher/classes')" class="h-10 w-10 rounded-2xl surface-card border border-card flex items-center justify-center text-muted hover:text-link hover:bg-[var(--accent-primary)]/10 transition-all">
        ...
        <div class="flex gap-3">
           <router-link :to="'/teacher/classes/' + classData.id + '/workspace'" class="rounded-xl bg-[var(--accent-primary)] px-4 py-3 text-sm font-semibold text-inverse hover:opacity-90 transition-all shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
       ...
       <div class="surface-card border border-card rounded-2xl p-4 group hover:border-link/30 transition-colors">
          <div class="flex justify-between items-center mb-4">
             <div class="h-10 w-10 rounded-2xl bg-[var(--accent-primary)]/10 text-link flex items-center justify-center">
       ...
       <div class="surface-card border border-card rounded-2xl p-4 group hover:border-link/30 transition-colors">
          <div class="flex justify-between items-center mb-4">
             <div class="h-10 w-10 rounded-2xl bg-[var(--color-info-bg)] text-[var(--color-info-text)] flex items-center justify-center">
...
<p class="text-xs font-semibold text-label mt-1">Buổi học sắp tới</p>
        ...
        <div class="surface-card border border-card rounded-2xl p-4 group hover:border-link/30 transition-colors">
           <div class="flex justify-between items-center mb-4">
              <div class="h-10 w-10 rounded-2xl bg-[var(--color-warning-bg)] text-[var(--color-warning-text)] flex items-center justify-center">
       ...
       <div class="surface-card border border-card rounded-2xl p-4 group hover:border-link/30 transition-colors">
          <div class="flex justify-between items-center mb-4">
             <div class="h-10 w-10 rounded-2xl bg-[var(--color-success-bg)] text-[var(--color-success-text)] flex items-center justify-center">
             ...
             <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <router-link to="/teacher/class-progress" class="group flex items-center justify-between p-5 rounded-2xl border border-card surface-card hover:surface-elevated hover:border-link/30 hover:shadow-md transition-all">
                   <div class="flex items-center gap-4">
                      <div class="h-10 w-10 rounded-2xl bg-[var(--accent-primary)]/10 text-link flex items-center justify-center group-hover:scale-110 transition-transform">
                ...
                <router-link to="/teacher/class-attendance" class="group flex items-center justify-between p-5 rounded-2xl border border-card surface-card hover:surface-elevated hover:border-link/30 hover:shadow-md transition-all">
                   <div class="flex items-center gap-4">
                      <div class="h-10 w-10 rounded-2xl bg-[var(--color-info-bg)] text-[var(--color-info-text)] flex items-center justify-center group-hover:scale-110 transition-transform">
                         <Clock :size="20" />
                      </div>
                      <div>
                ...
                <router-link to="/teacher/class-grades" class="group flex items-center justify-between p-5 rounded-2xl border border-card surface-card hover:surface-elevated hover:border-link/30 hover:shadow-md transition-all">
                   <div class="flex items-center gap-4">
                      <div class="h-10 w-10 rounded-2xl bg-[var(--accent-violet-soft)] text-[var(--accent-violet)] flex items-center justify-center group-hover:scale-110 transition-transform">
                         <TrendingUp :size="20" />
                      </div>
                      <div>
          ...
          <GlassPanel variant="soft" density="normal" class="h-full">
             <template #header>
               <div class="flex justify-between items-center">
                  <h2 class="text-lg font-semibold text-heading flex items-center gap-2">
                     <Bell :size="20" class="text-[var(--color-warning-text)]" /> Hoạt động mới
                  </h2>
             ...
             <div class="space-y-4">
                <div v-for="act in recentActivities" :key="act.id" class="relative flex items-center gap-3">
                   <div class="flex items-center justify-center w-10 h-10 rounded-full border-2 border-card surface-card text-muted shrink-0 shadow-sm z-10">
                      ...
                      <Clock v-if="act.type === 'leave'" :size="14" class="text-[var(--color-warning-text)]" />
                      <BookOpen v-if="act.type === 'material'" :size="14" class="text-[var(--color-success-text)]" />
                   </div>
