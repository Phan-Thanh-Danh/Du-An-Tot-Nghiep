...
const getCompletionColor = (status) => status === 'Completed' ? 'bg-green-500' : 'bg-slate-300'
...
<template>
  <div class="eval-page">
    ...
    <div class="privacy-banner">
      <ShieldCheck :size="20" class="text-green-600 shrink-0"/>
    ...
    <div class="eval-grid">
      <div v-for="ev in filteredEvals" :key="ev.id" class="eval-card" :class="{'card-completed': ev.status === 'Completed'}">
        <div class="ec-header">
          ...
          <div class="status-indicator" :class="ev.status === 'Completed' ? 'text-green-600' : 'text-slate-400'">
        ...
        <div class="ec-body">
          <div class="flex items-center gap-2 text-sm text-slate-700 font-semibold mb-2">
            <User :size="16" class="text-slate-400"/>
            {{ ev.teacher }}
          </div>
          <p class="text-xs text-slate-500 italic" v-if="ev.status === 'Completed'">Bạn còn {{ ev.editsLeft }} lần chỉnh sửa.</p>
        </div>
          ...
          <div class="modal-content lg">
            ...
            <div class="modal-body">
              <div class="eval-target-info">
                <div class="font-bold text-lg text-slate-900">{{ activeEval.subject }}</div>
                <div class="text-sm text-slate-600 flex items-center gap-1 mt-1"><User :size="14"/> Giảng viên: <strong>{{ activeEval.teacher }}</strong></div>
              </div>
        ...
        <div v-if="confirmModalOpen" class="modal-overlay" @click.self="confirmModalOpen = false">
          <div class="modal-content sm">
            ...
            <div class="modal-body text-center">
              <ShieldCheck :size="48" class="text-green-600 mx-auto mb-4" />
              <h4 class="text-lg font-bold text-slate-900 mb-2">Danh tính của bạn được bảo mật</h4>
              <p class="text-sm text-slate-600 mb-4">Bạn có chắc chắn muốn gửi đánh giá này không? Nội dung đánh giá sẽ được gửi ẩn danh hoàn toàn đến hệ thống và phân tích tự động bởi AI.</p>
              
              <div class="text-xs text-amber-700 bg-amber-50 p-2 rounded-lg border border-amber-200 text-left">
...
.select-trigger { display: flex; align-items: center; justify-content: space-between; padding: .6rem 1rem; border-radius: 10px; border: 1px solid rgba(148,163,184,.3); background: rgba(255,255,255,.8); font-size: .875rem; font-weight: 700; color: #0f172a; cursor: pointer; transition: all .2s; box-shadow: 0 2px 10px rgba(15,23,42,.05); backdrop-filter: blur(12px); }
.select-trigger:hover { background: #fff; border-color: #2563eb; color: #2563eb; }
.select-menu { position: absolute; top: calc(100% + .5rem); right: 0; width: 100%; background: rgba(255,255,255,.95); backdrop-filter: saturate(180%) blur(24px); border: 1px solid rgba(148,163,184,.2); border-radius: 16px; padding: .4rem; box-shadow: 0 10px 30px rgba(15,23,42,.12); display: flex; flex-direction: column; gap: .2rem; overflow: hidden; }
...
/* Banners */
.warning-banner { display: flex; align-items: flex-start; gap: 1rem; padding: 1.25rem 1.5rem; border-radius: 16px; backdrop-filter: blur(12px); box-shadow: 0 4px 20px rgba(220,38,38,.1); }
.blocker-warning { background: rgba(220,38,38,.08); border: 1px solid rgba(220,38,38,.2); color: #b91c1c; }
...
.privacy-banner { display: flex; align-items: center; gap: .75rem; background: rgba(22,163,74,.05); border: 1px solid rgba(22,163,74,.2); padding: .75rem 1rem; border-radius: 12px; }
...
.eval-card {
  ...
  transition: transform .2s, box-shadow .2s; border-left: 4px solid #ef4444;
}
.eval-card:hover { transform: translateY(-3px); box-shadow: 0 12px 30px rgba(15,23,42,.08); }
.card-completed { border-left-color: #22c55e; background: rgba(240,253,244,.4); }

.ec-header { padding: 1.25rem 1.25rem .75rem; border-bottom: 1px dashed rgba(148,163,184,.2); display: flex; flex-direction: column; gap: .5rem; }
.ec-subject { font-size: 1.1rem; font-weight: 800; color: #0f172a; line-height: 1.3; }
...
.ec-footer { padding: 1rem 1.25rem; border-top: 1px solid rgba(148,163,184,.15); background: rgba(248,250,252,.5); }
...
.modal-footer { padding: 1.25rem 1.5rem; border-top: 1px solid rgba(148,163,184,.15); display: flex; justify-content: flex-end; gap: .75rem; background: rgba(248,250,252,.5); flex-shrink: 0; }

/* Form Elements */
.eval-target-info { background: rgba(37,99,235,.05); border: 1px solid rgba(37,99,235,.2); padding: 1rem 1.25rem; border-radius: 12px; }
.criterion-item { border-bottom: 1px dashed rgba(148,163,184,.3); padding: 1.25rem 0; display: flex; flex-direction: column; gap: .75rem; }
.criterion-item:last-child { border-bottom: none; }
.crit-label { font-size: .95rem; font-weight: 700; color: #0f172a; margin: 0 0 .25rem; }
.crit-desc { font-size: .8125rem; color: #64748b; margin: 0; }
.crit-stars { display: flex; gap: .5rem; }
.star-btn { color: #cbd5e1; cursor: pointer; transition: all .2s; }
.star-btn:hover { transform: scale(1.1); }
.star-btn.active { color: #f59e0b; fill: #f59e0b; }

.input-glass { width: 100%; padding: .75rem 1rem; border-radius: 12px; border: 1px solid rgba(148,163,184,.4); background: rgba(255,255,255,.6); font-size: .875rem; outline: none; transition: border-color .2s; }
.input-glass:focus { border-color: #2563eb; }

.ai-notice { display: flex; gap: .5rem; background: rgba(124,58,237,.1); border: 1px solid rgba(124,58,237,.2); padding: .75rem 1rem; border-radius: 10px; color: #6d28d9; font-size: .8125rem; font-weight: 500; align-items: flex-start; }
