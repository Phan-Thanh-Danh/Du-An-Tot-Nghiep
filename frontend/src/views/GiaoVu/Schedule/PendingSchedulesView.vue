...
import {
  Clock, Filter, AlertCircle, Send, Eye, X, Loader2
...
async function loadData() {
  ...
  apiError.value = ''
  try {
    const res = await staffApi.getSchedules({ trangThai: 'pending' })
    schedules.value = res?.items ?? res ?? []
  } catch (err) {
    if (ENABLE_MOCK_API) {
      schedules.value = DEMO_SCHEDULES
    } else {
          ...
          <div v-for="item in schedules" :key="item.id"
            ...
            <div class="flex-1 pl-2">
              <div class="flex items-center gap-2 mb-1">
                ...
                <GlassBadge :variant="statusLabels[item.status].variant" size="xs">{{ statusLabels[item.status].label }}</GlassBadge>
              </div>
              ...
              <div>
                ...
                <div class="space-y-2 text-sm text-(--text-body)">
                  ...
                  <div class="flex justify-between"><span class="text-(--text-muted)">Trạng thái:</span>
                    <GlassBadge :variant="statusLabels[selectedItem.status].variant" size="xs">{{ statusLabels[selectedItem.status].label }}</GlassBadge>
                  </div>
                </div>
              </div>
