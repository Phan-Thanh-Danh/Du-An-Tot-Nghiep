...
async function fetchSubjects() {
  ...
  } catch (e) { error.value = e.message || 'Không thể tải danh sách môn học' }
...
async function submitForm() {
  ...
  try {
    ...
    closeForm(); await fetchSubjects()
  } catch (e) { formErrors.value._api = e.message || 'Lỗi khi lưu môn học' }
...
async function executeDelete() {
  ...
  try { await subjectApi.deactivate(confirmDelete.value.maMonHoc); confirmDelete.value = null; await fetchSubjects() }
