  ...
  <GlassPanel density="none" class="rounded-[28px]">
    <div class="flex items-center justify-between gap-3 border-b border-white/45 px-4 py-3.5">
      <div>
        ...
        <p class="text-xs font-medium text-slate-500">Học tập & Hoạt động</p>
      </div>
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <div class="flex items-start justify-between gap-2">
            <p :class="['truncate text-[13px] leading-tight', notification.unread ? 'font-bold text-slate-950' : 'font-semibold text-slate-500']">{{ notification.title }}</p>
          ...
          <div class="mt-1 flex items-center gap-2.5">
            ...
            <span class="text-xs font-medium text-slate-400">{{ notification.time }}</span>
          </div>
        </div>
      </button>
    </div>
    <div class="border-t border-white/45 bg-white/30 px-5 py-3">
      <router-link to="/student/notifications" class="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 transition-colors hover:text-blue-800">
        Xem tất cả
      </router-link>
    </div>
  </GlassPanel>
