    ...
    <div class="space-y-2 p-4">
      <article
        ...
        :class="[
          ...
          item.current
            ? 'border-[color-mix(in srgb,var(--text-link) 20%,transparent)] bg-(--color-info-bg) shadow-md ring-1 ring-[color-mix(in srgb,var(--text-link) 20%,transparent)]'
      ...
      >
          ...
          <span
            :class="[
              ...
              item.current ? 'bg-(--text-link)' : 'bg-(--text-placeholder)',
            ]"
          />
          <span class="mt-1 h-full w-px bg-gradient-to-b from-(--text-link)/20 to-transparent" />
        </div>
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="inline-flex rounded-full border border-[color-mix(in srgb,var(--text-link) 20%,transparent)] bg-(--surface-card) px-2 py-0.5 text-xs font-semibold text-link shadow-sm">
                {{ item.time }}
              </p>
