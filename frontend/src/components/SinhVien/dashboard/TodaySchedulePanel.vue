  ...
  <GlassPanel density="none" class="rounded-[28px]">
    <div class="flex items-center justify-between gap-3 border-b border-white/45 px-4 py-3.5">
      <div>
        ...
        <p class="text-xs font-medium text-slate-500">Thứ sáu, 15/05/2026</p>
      </div>
    ...
    <div class="space-y-2 p-4">
      ...
      >
        <div class="flex flex-col items-center">
          ...
          />
          <span class="mt-1 h-full w-px bg-gradient-to-b from-blue-200 to-transparent" />
        </div>
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="inline-flex rounded-full border border-blue-50 bg-white/80 px-2 py-0.5 text-xs font-semibold text-blue-700 shadow-sm">
                {{ item.time }}
              </p>
              <span v-if="item.current" class="flex h-1.5 w-1.5 rounded-full bg-blue-600" />
            </div>
            <h3 class="mt-1.5 truncate text-[13px] font-bold text-slate-950 leading-tight">{{ item.subject }}</h3>
            <p class="mt-1 flex items-center gap-1.5 truncate text-xs font-medium text-slate-500">
              <MapPin :size="11" />
