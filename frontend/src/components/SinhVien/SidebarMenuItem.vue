...
<template>
  <router-link
    ...
    @click="handleClick"
    :class="[
      'lg-sidebar-item group relative flex min-h-8 w-full items-center gap-2.5 rounded-(--radius-md) px-2.5 py-1.5 text-[13px] font-medium transition-all duration-200 focus:outline-none focus:ring-(--sidebar-focus-ring)',
      depth === 0
      ...
      isActive
        ? depth === 0
          ...
          : 'border-card bg-(--surface-card-hover) text-(--sidebar-accent) font-semibold shadow-sm'
        : '',
    ]"
  >
