...
<div style="font-family:'Segoe UI',Arial,sans-serif;color:#1E293B;background:#fff;width:100%;max-width:794px;margin:0 auto;padding:0;">
  ...
  const worker = html2pdf().set({
    margin: [10, 5, 10, 5],
    filename: `BaoCao-TongQuan-${semesterLabel.replace(/[^a-zA-Z0-9\u00C0-\u024F]/g, '_')}.pdf`,
...
/**
 * Xuất báo cáo GPA Hệ thống ra file PDF.
 * Dùng <table> layout thuần để tránh lỗi html2canvas.
 */
export async function exportGpaReportToPdf(opts = {}) {
  const html2pdf = (await import('html2pdf.js')).default
  ...
  const kpiCells = `
    <td style="width:33.33%;padding:4px;">
      <div style="background:#EFF6FF;border:1.5px solid #BFDBFE;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">GPA TRUNG BÌNH</p>
        <p style="font-size:20px;font-weight:800;color:#1D4ED8;margin:0;">${Number(overallAvgGpa).toFixed(2)}</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#F0FDF4;border:1.5px solid #BBF7D0;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">TỶ LỆ KHÁ GIỎI (>= 3.2)</p>
        <p style="font-size:20px;font-weight:800;color:#15803D;margin:0;">${highGpaRate}%</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#FFFBEB;border:1.5px solid #FDE68A;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">GPA CAO NHẤT</p>
        <p style="font-size:20px;font-weight:800;color:#B45309;margin:0;">${maxGpaValue}</p>
      </div>
    </td>`

  // ── Data rows ──────────────────────────────────────
  const dataRows = filteredStats.map((s, idx) => {
    return `<tr style="background:${idx % 2 === 0 ? '#F8FAFC' : '#FFFFFF'};">
      <td style="padding:7px 10px;font-weight:600;color:#334155;font-size:11px;">${s.group}</td>
      <td style="padding:7px 10px;text-align:center;color:#1D4ED8;font-weight:700;font-size:11px;">${s.avgGpa.toFixed(2)}</td>
      <td style="padding:7px 10px;text-align:center;color:#64748B;font-size:11px;">${s.minGpa.toFixed(2)}</td>
      <td style="padding:7px 10px;text-align:center;color:#64748B;font-size:11px;">${s.maxGpa.toFixed(2)}</td>
      <td style="padding:7px 10px;text-align:center;color:#B91C1C;font-weight:600;font-size:11px;">${s.warningCount}</td>
      <td style="padding:7px 10px;text-align:right;color:#64748B;font-size:11px;">${s.campus}</td>
    </tr>`
  }).join('')
...
<div style="font-family:'Segoe UI',Arial,sans-serif;color:#1E293B;background:#fff;width:100%;max-width:794px;margin:0 auto;padding:0;">
  ...
  <div style="background:linear-gradient(135deg,#1E3A8A 0%,#2563EB 60%,#0EA5E9 100%);padding:28px 36px 24px;">
    <table style="width:100%;border-collapse:collapse;">
      <tr>
        <td style="vertical-align:top;">
          <p style="font-size:10px;font-weight:600;color:#93C5FD;text-transform:uppercase;letter-spacing:1px;margin:0 0 5px;">BÁO CÁO HỌC VỤ — HỆ THỐNG LMS</p>
          <p style="font-size:20px;font-weight:800;color:#fff;margin:0 0 3px;">Báo cáo GPA Hệ thống</p>
          <p style="font-size:11px;color:#BAE6FD;margin:0;">Phân tích điểm trung bình tích lũy theo lớp/khoa/cơ sở</p>
        </td>
        <td style="vertical-align:top;text-align:right;white-space:nowrap;">
          <p style="font-size:10px;color:#93C5FD;margin:0 0 3px;">Ngày xuất: ${dateStr} lúc ${timeStr}</p>
          <p style="font-size:10px;color:#BAE6FD;margin:0 0 2px;">Học kỳ: ${semesterLabel}</p>
          <p style="font-size:10px;color:#BAE6FD;margin:0 0 2px;">Ngành: ${industryLabel}</p>
          <p style="font-size:10px;color:#BAE6FD;margin:0;">Cơ sở: ${campusLabel}</p>
        </td>
      </tr>
    </table>
  ...
  <div style="padding:24px 36px;">
    ...
    <div style="border:1.5px solid #E2E8F0;border-radius:10px;overflow:hidden;margin-bottom:20px;">
      <div style="background:#F8FAFC;padding:10px 14px;border-bottom:1px solid #E2E8F0;">
        <p style="font-size:11px;font-weight:700;color:#334155;text-transform:uppercase;letter-spacing:0.4px;margin:0;">Chi tiết điểm số theo Khoa / Lớp</p>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#F1F5F9;">
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Khoa / Lớp</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">GPA TB</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Min GPA</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Max GPA</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Dưới ngưỡng</th>
          <th style="padding:7px 10px;text-align:right;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Cơ sở</th>
        </tr></thead>
        <tbody>${dataRows || '<tr><td colspan="6" style="text-align:center;padding:14px;color:#94A3B8;font-size:11px;">Chưa có dữ liệu</td></tr>'}</tbody>
      </table>
    </div>
  ...
  const worker = html2pdf().set({
    margin: [10, 5, 10, 5],
    filename: `BaoCao-GPA-${semesterLabel.replace(/[^a-zA-Z0-9\u00C0-\u024F]/g, '_')}.pdf`,
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, logging: false },
    jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
  }).from(html)
...
/**
 * Xuất báo cáo Sinh viên nguy cơ rớt môn ra file PDF.
 */
export async function exportAtRiskToPdf(opts = {}) {
  const html2pdf = (await import('html2pdf.js')).default
  ...
  const kpiCells = `
    <td style="width:33.33%;padding:4px;">
      <div style="background:#FEF2F2;border:1.5px solid #FECACA;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">TỔNG SV NGUY CƠ</p>
        <p style="font-size:20px;font-weight:800;color:#B91C1C;margin:0;">${totalAtRisk}</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#FFFBEB;border:1.5px solid #FDE68A;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">TỶ LỆ CRITICAL</p>
        <p style="font-size:20px;font-weight:800;color:#B45309;margin:0;">${criticalRate}%</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#FEF2F2;border:1.5px solid #FECACA;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">CẦN CAN THIỆP</p>
        <p style="font-size:20px;font-weight:800;color:#B91C1C;margin:0;">${criticalCount} SV</p>
      </div>
    </td>`

  // ── Data rows ──────────────────────────────────────
  const dataRows = students.map((s, idx) => {
    const riskBadge = s.risk === 'critical' ? 'background:#FEF2F2;color:#B91C1C;' : s.risk === 'high' ? 'background:#FFFBEB;color:#B45309;' : 'background:#EFF6FF;color:#1D4ED8;'
    return `<tr style="background:${idx % 2 === 0 ? '#F8FAFC' : '#FFFFFF'};">
      <td style="padding:7px 10px;text-align:center;color:#64748B;font-size:11px;">${idx + 1}</td>
      <td style="padding:7px 10px;font-weight:600;color:#334155;font-size:11px;">${s.code}</td>
      <td style="padding:7px 10px;font-weight:600;color:#334155;font-size:11px;">${s.name}</td>
      <td style="padding:7px 10px;text-align:center;color:#64748B;font-size:11px;">${s.class}</td>
      <td style="padding:7px 10px;color:#64748B;font-size:11px;">${s.subject || 'Đang cập nhật'}</td>
      <td style="padding:7px 10px;text-align:center;font-weight:700;color:${s.grade < 4 ? '#B91C1C' : '#B45309'};font-size:11px;">${s.grade}</td>
      <td style="padding:7px 10px;text-align:center;">
        <span style="${riskBadge}font-weight:800;padding:2px 8px;border-radius:20px;font-size:9px;text-transform:uppercase;">${s.risk}</span>
      </td>
    </tr>`
  }).join('')
...
<div style="font-family:'Segoe UI',Arial,sans-serif;color:#1E293B;background:#fff;width:100%;max-width:794px;margin:0 auto;padding:0;">
  ...
  <div style="background:linear-gradient(135deg,#1E3A8A 0%,#2563EB 60%,#0EA5E9 100%);padding:28px 36px 24px;">
    <table style="width:100%;border-collapse:collapse;">
      <tr>
        <td style="vertical-align:top;">
          <p style="font-size:10px;font-weight:600;color:#93C5FD;text-transform:uppercase;letter-spacing:1px;margin:0 0 5px;">BÁO CÁO HỌC VỤ — HỆ THỐNG LMS</p>
          <p style="font-size:20px;font-weight:800;color:#fff;margin:0 0 3px;">Báo cáo Sinh viên nguy cơ rớt môn</p>
          <p style="font-size:11px;color:#BAE6FD;margin:0;">Hệ thống cảnh báo sớm AI Early Warning</p>
        </td>
        <td style="vertical-align:top;text-align:right;white-space:nowrap;">
          <p style="font-size:10px;color:#93C5FD;margin:0 0 3px;">Ngày xuất: ${dateStr} lúc ${timeStr}</p>
  ...
  <div style="padding:24px 36px;">
    ...
    <div style="border:1.5px solid #E2E8F0;border-radius:10px;overflow:hidden;margin-bottom:20px;">
      <div style="background:#F8FAFC;padding:10px 14px;border-bottom:1px solid #E2E8F0;">
        <p style="font-size:11px;font-weight:700;color:#334155;text-transform:uppercase;letter-spacing:0.4px;margin:0;">Danh sách sinh viên (${students.length} dòng)</p>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#F1F5F9;">
          ...
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Mã SV</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Họ và tên</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Lớp</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Môn nguy cơ</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">GPA</th>
          <th style="padding:7px 10px;text-align:center;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Mức độ</th>
        </tr></thead>
        <tbody>${dataRows || '<tr><td colspan="7" style="text-align:center;padding:14px;color:#94A3B8;font-size:11px;">Không có sinh viên nào</td></tr>'}</tbody>
      </table>
    </div>
  ...
  const worker = html2pdf().set({
    margin: [10, 5, 10, 5],
    filename: `BaoCao-SVNguyCo-${semesterLabel.replace(/[^a-zA-Z0-9\u00C0-\u024F]/g, '_')}.pdf`,
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, logging: false },
    jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
  }).from(html)
...
/**
 * Xuất Báo cáo học tập chi tiết ra file PDF.
 */
export async function exportAcademicReportsToPdf(opts = {}) {
  const html2pdf = (await import('html2pdf.js')).default
  ...
  const classKpiCells = `
    <td style="width:33.33%;padding:4px;">
      <div style="background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">LỚP HỌC PHẦN ĐANG MỞ</p>
        <p style="font-size:20px;font-weight:800;color:#0F172A;margin:0;">${tabTotalClasses}</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">GIẢNG VIÊN PHỤ TRÁCH</p>
        <p style="font-size:20px;font-weight:800;color:#0F172A;margin:0;">${tabTotalTeachers}</p>
      </div>
    </td>
    <td style="width:33.33%;padding:4px;">
      <div style="background:#F8FAFC;border:1.5px solid #E2E8F0;border-radius:10px;padding:14px 10px;text-align:center;">
        <p style="font-size:9px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:0.4px;margin:0 0 6px;">GPA TB THEO LỚP</p>
        <p style="font-size:20px;font-weight:800;color:#0F172A;margin:0;">${formatGpa(tabAvgGpa)}</p>
      </div>
    </td>`

  // ── Monthly Stats rows ──────────────────────────────────────
  const monthlyRows = monthlyStats.map((m, idx) => {
    return `<tr style="background:${idx % 2 === 0 ? '#F8FAFC' : '#FFFFFF'};">
      <td style="padding:7px 10px;font-weight:600;color:#334155;font-size:11px;">${m.semester}</td>
      <td style="padding:7px 10px;color:#64748B;font-size:11px;">${m.totalGrades} lượt</td>
      <td style="padding:7px 10px;font-weight:700;color:#15803D;font-size:11px;">${m.passCount} Pass</td>
      <td style="padding:7px 10px;font-weight:700;color:#B91C1C;font-size:11px;">${m.failCount} Fail</td>
      <td style="padding:7px 10px;font-weight:700;color:#0F172A;font-size:11px;">${formatGpa(m.avgGpa)}</td>
    </tr>`
  }).join('')

  // ── Dept Stats rows ──────────────────────────────────────
  const deptRows = departmentStats.map((d, idx) => {
    return `<tr style="background:${idx % 2 === 0 ? '#F8FAFC' : '#FFFFFF'};">
      <td style="padding:7px 10px;font-weight:600;color:#334155;font-size:11px;">${d.departmentName}</td>
      <td style="padding:7px 10px;color:#64748B;font-size:11px;">${d.totalGrades} bài</td>
      <td style="padding:7px 10px;font-weight:700;color:#15803D;font-size:11px;">${d.passCount} Pass</td>
      <td style="padding:7px 10px;font-weight:700;color:#0284C7;font-size:11px;">${d.passRate}%</td>
      <td style="padding:7px 10px;font-weight:700;color:#0F172A;font-size:11px;">${formatGpa(d.avgGpa)}</td>
    </tr>`
  }).join('')
...
<div style="font-family:'Segoe UI',Arial,sans-serif;color:#1E293B;background:#fff;width:100%;max-width:794px;margin:0 auto;padding:0;">
  ...
  <div style="background:linear-gradient(135deg,#1E3A8A 0%,#2563EB 60%,#0EA5E9 100%);padding:28px 36px 24px;">
    <table style="width:100%;border-collapse:collapse;">
      <tr>
        <td style="vertical-align:top;">
          <p style="font-size:10px;font-weight:600;color:#93C5FD;text-transform:uppercase;letter-spacing:1px;margin:0 0 5px;">BÁO CÁO HỌC VỤ — HỆ THỐNG LMS</p>
          <p style="font-size:20px;font-weight:800;color:#fff;margin:0 0 3px;">Báo cáo học tập chi tiết</p>
          <p style="font-size:11px;color:#BAE6FD;margin:0;">Phân tích đa chiều theo lớp, môn học và cơ sở đào tạo</p>
        </td>
        <td style="vertical-align:top;text-align:right;white-space:nowrap;">
          <p style="font-size:10px;color:#93C5FD;margin:0 0 3px;">Ngày xuất: ${dateStr} lúc ${timeStr}</p>
          <p style="font-size:10px;color:#BAE6FD;margin:0 0 3px;">Học kỳ: ${semesterLabel}</p>
          <p style="font-size:10px;color:#BAE6FD;margin:0;">Cơ sở: ${campusLabel}</p>
        </td>
      </tr>
    </table>
  ...
  <div style="padding:24px 36px;">
    ...
    <!-- TỔNG QUAN -->
    <h3 style="font-size:12px;font-weight:800;color:#0F172A;text-transform:uppercase;margin:0 0 10px;">1. Báo cáo Tóm tắt (Lớp học phần)</h3>
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;"><tr>${classKpiCells}</tr></table>

    <!-- BẢNG KẾT QUẢ THEO LỚP -->
    <h3 style="font-size:12px;font-weight:800;color:#0F172A;text-transform:uppercase;margin:0 0 10px;">2. Thống kê kết quả theo Học kỳ / Đơn vị</h3>
    <div style="border:1.5px solid #E2E8F0;border-radius:10px;overflow:hidden;margin-bottom:24px;">
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#F1F5F9;">
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Học kỳ / Đơn vị</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Tổng lượt</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Pass</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Fail</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">GPA TB</th>
        </tr></thead>
        <tbody>${monthlyRows || '<tr><td colspan="5" style="text-align:center;padding:14px;color:#94A3B8;font-size:11px;">Không có dữ liệu</td></tr>'}</tbody>
      </table>
    </div>

    <!-- BẢNG KẾT QUẢ THEO KHOA -->
    <h3 style="font-size:12px;font-weight:800;color:#0F172A;text-transform:uppercase;margin:0 0 10px;">3. Thống kê kết quả theo Khoa / Chuyên ngành</h3>
    <div style="border:1.5px solid #E2E8F0;border-radius:10px;overflow:hidden;margin-bottom:24px;">
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#F1F5F9;">
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Khoa / Ngành đào tạo</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Tổng lượt</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Số bài đạt</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">Tỷ lệ Pass</th>
          <th style="padding:7px 10px;text-align:left;color:#64748B;font-size:9px;font-weight:700;text-transform:uppercase;">GPA TB</th>
        </tr></thead>
        <tbody>${deptRows || '<tr><td colspan="5" style="text-align:center;padding:14px;color:#94A3B8;font-size:11px;">Không có dữ liệu</td></tr>'}</tbody>
      </table>
    </div>
  ...
  const worker = html2pdf().set({
    margin: [10, 5, 10, 5],
    filename: `BaoCao-HocTapChiTiet-${semesterLabel.replace(/[^a-zA-Z0-9\u00C0-\u024F]/g, '_')}.pdf`,
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, logging: false },
    jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
  }).from(html)
