...
<template>
  <aside
    ...
    :style="{
      '--sidebar-glow-1': 'rgba(234, 88, 12, 0.10)',
      '--sidebar-glow-2': 'rgba(249, 115, 22, 0.07)',
      '--sidebar-glow-dark-1': 'rgba(234, 88, 12, 0.07)',
      '--sidebar-glow-dark-2': 'rgba(249, 115, 22, 0.05)',
      '--sidebar-accent': '#ea580c',
      '--sidebar-accent-dark': '#f97316',
      '--sidebar-indicator': '#ea580c',
      '--sidebar-focus-ring': 'rgba(234, 88, 12, 0.2)',
      '--active-glow': 'rgba(255, 255, 255, 0.22)',
      '--active-start': '#c2410c',
      '--active-mid': '#ea580c',
      '--active-end': '#fb923c',
      '--active-shadow-1': 'rgba(194, 65, 12, 0.20)',
      '--active-shadow-2': 'rgba(249, 115, 22, 0.14)',
    }"
  >
    <!-- ──────────── LOGO / BRAND ──────────── -->
    <div
    ...
    >
      <div class="lg-sidebar-brand-mark flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-[var(--radius-lg)] bg-gradient-to-tr from-orange-600 to-amber-500">
      ...
      <Transition name="fade-slide">
        <div v-if="!collapsed" class="overflow-hidden">
          ...
          <p class="text-[10px] leading-tight text-muted">Phụ huynh</p>
        </div>
      </Transition>
    </div>
    ...
    <nav class="relative flex-1 space-y-0.5 overflow-y-auto overflow-x-visible px-2 py-2 scrollbar-thin">
      <SidebarMenuGroup
        v-for="group in phuHuynhMenuGroups"
        :key="group.id"
        :group="group"
        :collapsed="collapsed"
      />
    ...
    >
      <div :class="['lg-nav flex items-center gap-2 rounded-xl p-2', collapsed ? '' : 'w-full']">
        <div class="lg-sidebar-avatar relative flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ring-1 ring-[var(--border-card)] bg-orange-100 text-orange-700">
