...
function isTabActive(tabPath) {
  if (tabPath === '/parent/dashboard') {
    return route.path === tabPath
  }
  const tabPrefix = tabPath.split('/').slice(0, 3).join('/')
  return route.path.startsWith(tabPrefix)
}
...
<template>
  ...
  >
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 lg:hidden lg-glass border-t border-card py-2.5 px-3 flex items-center justify-around pb-safe">
      <router-link
        v-for="tab in mobileTabs"
        :key="tab.path"
        :to="tab.path"
        class="flex flex-col items-center gap-1 py-1 px-3.5 rounded-xl transition-all duration-200"
        :class="isTabActive(tab.path) ? 'text-orange-600 font-bold' : 'text-muted'"
      >
        <component :is="tab.icon" :size="19" :class="isTabActive(tab.path) ? 'scale-110 text-orange-600' : 'text-muted'" />
        <span class="text-[9px] font-semibold tracking-tight">{{ tab.label }}</span>
      </router-link>
    </div>
  </div>
  <AiAssistant />
