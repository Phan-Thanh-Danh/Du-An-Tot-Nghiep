...
import AdminAssignmentPanel from './AdminAssignmentPanel.vue'
import AdminDecisionPanel from './AdminDecisionPanel.vue'
...
const parsedFormData = computed(() => {
  try { return JSON.parse(props.application.duLieuForm || '{}') } catch { return {} }
})
const schemaPlaceholder = computed(() => {
  // Mock schema extraction for view-only rendering
  return Object.keys(parsedFormData.value).map(k => ({
...
<template>
  <div class="h-full flex flex-col space-y-4">
    ...
    <div class="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
      ...
      <!-- Sidebar -->
      <div class="space-y-6 overflow-y-auto custom-scrollbar pr-2 pb-6">
        <AdminAssignmentPanel :application="application" :assignableUsers="[]" />
        <AdminDecisionPanel :application="application" />
      </div>
    </div>
  </div>
</template>
