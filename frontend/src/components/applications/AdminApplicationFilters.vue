...
<template>
  <div class="flex flex-col sm:flex-row gap-4 mb-6 p-4 bg-[var(--surface-card)] border border-[var(--border-card)] rounded-xl">
    <div class="flex-1 relative">
      <Search class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" />
      <input 
        v-model="filters.keyword"
        @keyup.enter="applyFilters"
        class="w-full h-10 pl-9 pr-3 rounded-lg border border-[var(--border-input)] bg-[var(--surface-input)] text-[var(--text-body)] focus:border-[var(--lg-primary)] outline-none"
        placeholder="Tìm theo mã đơn, tiêu đề hoặc MSSV..."
      />
    </div>
    
    <div class="w-full sm:w-48">
      <select v-model="filters.status" @change="applyFilters" class="w-full h-10 px-3 rounded-lg border border-[var(--border-input)] bg-[var(--surface-input)] text-[var(--text-body)]">
