...
public class CaHocService : ICaHocService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        EnsureCanManageCaHoc(currentUser);

        var tenCa = NormalizeRequiredText(request.TenCa, "Tên ca");
        var buoi = NormalizeSession(request.Buoi);
        var gioBatDau = ParseTime(request.GioBatDau, "Giờ bắt đầu");
        var gioKetThuc = ParseTime(request.GioKetThuc, "Giờ kết thúc");
        ValidateTimeRange(gioBatDau, gioKetThuc);
        ValidateDisplayOrder(request.ThuTu);
        await ValidateUniqueNameAsync(tenCa, null, cancellationToken);
        ...
        var newSnapshot = ToAuditSnapshot(shift);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ValidateTimeRange(gioBatDau, gioKetThuc);
        ValidateDisplayOrder(request.ThuTu);
        await ValidateUniqueNameAsync(tenCa, shiftId, cancellationToken);

        if ((oldStartTime != gioBatDau || oldEndTime != gioKetThuc) &&
            await IsShiftUsedAsync(shiftId, cancellationToken))
        {
            throw new ApiException(
                StatusCodes.Status400BadRequest,
                "Không được sửa giờ bắt đầu/kết thúc vì ca học đã được dùng trong thời khóa biểu hoặc buổi học.");
        }

        shift.TenCa = tenCa;
        shift.Buoi = buoi;
        shift.GioBatDau = gioBatDau;
        shift.GioKetThuc = gioKetThuc;
    ...
    public async Task<CaHocDto> ToggleActiveAsync(int shiftId, CancellationToken cancellationToken = default)
    {
        ...
        await WriteAuditAsync(
            ...
            newSnapshot,
            currentUser,
            shift.ConHoatDong ? "Bật hoạt động ca học." : "Tắt hoạt động ca học.",
            cancellationToken);
    ...
    private async Task<bool> IsShiftUsedAsync(int shiftId, CancellationToken cancellationToken)
    {
        return await _context.ThoiKhoaBieus
            .AsNoTracking()
            .AnyAsync(x => x.MaCaHoc == shiftId, cancellationToken) ||
            await _context.BuoiHocs
                .AsNoTracking()
                .AnyAsync(x => x.MaCaHoc == shiftId, cancellationToken);
    }
    ...
    private static void EnsureCanManageCaHoc(CurrentUserContext currentUser)
    {
    ...
    private static string NormalizeSession(string value)
    {
        ...
        {
            throw new ApiException(StatusCodes.Status400BadRequest, "Buổi học không hợp lệ. Giá trị hợp lệ: sang, chieu, toi.");
        }
    ...
    private static TimeOnly ParseTime(string value, string fieldName)
    {
        ...
        if (!TimeOnly.TryParseExact(
                ...
                out var time))
        {
            throw new ApiException(StatusCodes.Status400BadRequest, $"{fieldName} phải có định dạng HH:mm hoặc HH:mm:ss.");
        }
