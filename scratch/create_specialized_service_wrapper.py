...
public class SpecializedNotificationService : ISpecializedNotificationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!actor.IsGlobal)
        {
            ...
            query = query.Where(x => x.MaDonVi != null && allowed.Contains(x.MaDonVi.Value));
        }
            ...
            case "major":
                if (target.MaNganh.HasValue)
                {
                    var lopIds = await _dbContext.LopHanhChinhs.AsNoTracking()
                        .Where(l => l.ChuongTrinhDaoTao != null && 
                                    l.ChuongTrinhDaoTao.ChuyenNganh != null && 
                                    l.ChuongTrinhDaoTao.ChuyenNganh.MaNganh == target.MaNganh.Value)
                        .Select(l => l.MaLop)
                        .ToListAsync(cancellationToken);
...
"""
with open("scratch/create_specialized_service.py", "w", encoding="utf-8") as f:
    f.write(f"""import os

content = r'''{content}'''

with open('Backend/Services/Notification/SpecializedNotificationService.cs', 'w', encoding='utf-8') as f:
    f.write(content)
""")
